...
- **Vault visualizer:** built (`tools/build_view.py`, 2026-07-21). **Read-only, frontmatter-only** generator that emits ONE self-contained interactive HTML file, `views/relatio-view.html` (a new top-level directory beside `tools/`, **outside the KOS numbering and lifecycle** — a generated artifact, same category as CLAUDE.md; committed so it is shareable from the public repo). It parses YAML frontmatter only — never body prose — writes to no record, and adds no fields; an absent datum shows "not fielded" honestly. Renders the INV → claims/findings (`part_of`) → sources (`derived_from`) tree with every confidence component shown separately (never merged/averaged), a past-due `review_date` marker (a second review-queue surface), and typed cross-edges as chips. **The reliance gate is load-bearing: a standing banner plus an R0/R1/R2 badge on every CLM/FND node — never remove it from the generator; no view/filter/search path may strip it** (the `main()` self-check fails if the badge count ≠ the CLM/FND count). Closure state is **not** derivable from frontmatter (it lives in prose banners; closed INVs stay `status: Draft`), so INV nodes are labelled by their honest `status`/`operational_status` only. **Regenerate after vault changes (`python build_view.py`); never hand-edit the `views/` output.** Validators ignore `views/` (outside the vault dir, no markdown). Details in `tools/README.md`.
