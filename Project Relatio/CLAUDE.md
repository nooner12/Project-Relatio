...
- **Version-coherence automation:** built (GB-2026-035, 2026-07-20) — `validate.py` now checks that a document's `version:` frontmatter, its `## Version` heading, and the newest row of its `# Revision History` table agree, reporting mismatches as **warnings**. **Do not reconcile versions by hand — run the tool.** The vault's warning baseline is **zero**, so any warning is a real delta. Two rules the check enforces and you should follow when editing by hand: the **newest history row is found by parsing and taking the max, never by position** (rows are not always in order — GB-2026-029), and **an edit that bumps a version must also write its history row** (the 40th drift instance, ROLE-0003, was exactly that omission).
- **Investigation closure state (as of 2026-07-20):** **INV-0001…INV-0010, INV-0012, INV-0013 are formally CLOSED; INV-0011 alone remains open**, blocked solely on its criterion #2 (three sub-questions per doctrine claim) — substantive research work, which ADR-GOV-0005 §2 expressly declines to relax. "Closed" means the inquiry is complete; it is **not** a maturity promotion (INVs stay `status: Draft`) and **not** a clearance for external reliance.
- **Any tool that enumerates the vault MUST use extended-length path handling** (`\\?\` or equivalent) — four files exceed the Windows 260-char `MAX_PATH` and are invisible to naive scanners; a scan without it is non-conforming and its results invalid *even when it reports clean*. Rule and the ≤180-char relative-path budget: **STD-0001 §8** (Path Length Constraint).
