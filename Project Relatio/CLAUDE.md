...
- **Investigation closure state (as of 2026-07-21):** **all thirteen investigations INV-0001…INV-0013 are formally CLOSED; none open.** INV-0011 — the last — closed 2026-07-21 after an owner-directed sub-question completion circuit met its criterion #2 (explicit, separable (i)/(ii)/(iii) answers in each doctrine claim, CLM-0051…CLM-0055, full OPS-0003 circuit; met at INV-0011 v0.5, not v0.2). "Closed" means the inquiry is complete; it is **not** a maturity promotion (INVs stay `status: Draft`) and **not** a clearance for external reliance.
