...
1. [ ] The question is decomposed into the **six mandated claims** (§1), each separately gradable.
2. [ ] For **each** claim, elements **(a)/(b)/(c)** are answered **explicitly and separably in the claim record** — discrete sections, not diffused prose.
3. [ ] Every source is **verification-disclosed** per §5.7; **UNSOURCED is a recordable outcome**; anti-fabrication (KOS-0003 §12.1) is absolute; **no untranslated material paraphrased from recall**.
4. [ ] **Native `Level N (Label)` is the only on-record confidence vocabulary** — no H-band and no ★ in any frontmatter or grading field anywhere in the KB objects.
5. [ ] The **dual-track discipline (§2.1) held**: self-understanding graded as fact-about-the-movement, never covertly settling — and never dismissed on — the classification.
6. [ ] The **classification-evidence rule (§3.1) is observed**: claim 3 answers on scholarly grounds; claims 4/5 inform, don't settle.
...
8. [ ] **Standards, Appendix A, and the Architecture Baseline are unmodified; the Iranian source base (SRC-0104…SRC-0123) is untouched.**
